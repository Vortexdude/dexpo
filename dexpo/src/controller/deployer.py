...
class DeployHandler:
    ...
    @staticmethod
    def set_state_data(resource_type: str, resource_name: str, resource_id: str, obj: object):
        state.data.update({resource_type: state.formatter(_id=resource_id, resource=obj, name=resource_name)})

    @staticmethod
    def set_state_data_list(resource_type: str, data: dict):
        state.data.update({resource_type: data})

    def get_state_data(self, resource):
    ...
    def create_vpc(self):
        if 'VpcId' not in self.vpc:
            ...
            self.set_state_data(resource_id=vpc_id, resource_type='vpc', resource_name=self.vpc_name,
                                obj=vpc_resource)
    ...
    def create_internet_gateway(self):
        ...
        if 'InternetGatewayId' not in self.internet_gateway:
            ...
            vpc_resource = self.get_state_data('vpc_resource')
            ...
            self.set_state_data(resource_name=ig_name, resource_type='ig', resource_id=ig_id, obj=ig_resource)
            self.internet_gateway.update(response)
        else:
            logger.info(f"Internet Gateway {ig_name} already exists")

    def create_route_table(self):
        self._create_resource(self.route_tables, 'RouteTableId', 'rt', self.x_rt)

    def create_subnets(self):
        self._create_resource(self.subnets, 'SubnetId', 'sb', self.x_sb)

    def create_security_groups(self):
        self._create_resource(self.security_groups, 'GroupId', 'sg', self.x_sg)

    def _create_resource(self, resource_list: list, identifier: str, resource_type: str, function):
        ...
        for resource in resource_list:
            if identifier not in resource:
                ...
                res = identifier.split("Id")[0]
                logger.info(f"{res} {resource['name']} already exists")

        if global_resource_data != {}:
            self.set_state_data_list(resource_type=resource_type, data=global_resource_data)
    ...
    def x_rt(self, data):
        ...
        vpc_resource = self.get_state_data('vpc_resource')
        ig_id = self.get_state_data('ig_id')
        if not data['DestinationCidrBlock']:  # check for private route that doesn't have the public route
            ig_id = ''
            logger.info(f"Route Table {data['name']} will not have any public route")
    ...
    def x_sg(self, data):
        ...
        vpc_id = self.get_state_data('vpc_id')
